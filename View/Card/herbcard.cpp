...
HerbCard::HerbCard(const Herb& aValue, QWidget* parent)
    : value{aValue}, QWidget{parent}
{
    ...
    QVBoxLayout* herbInfo{new QVBoxLayout};

    name = new QLabel{QString::fromStdString(Utility::displayfy(value.getName()))};
    name->setMargin(4);
    name->setObjectName("nameHerbCard");
    name->setMaximumHeight(80);


    type = new QLabel;
    QImage typeImage{":/Img/Images/Icons/basil.png"};
    ...
    lff->setMaximumSize(400, 150);

    std::vector<QString> v2;
    if (value.getUses()[0])
        v2.push_back(":/Img/Images/Icons/salad.png");
    if (value.getUses()[1])
        v2.push_back(":/Img/Images/Icons/lotion.png");
    if (value.getUses()[2])
        v2.push_back(":/Img/Images/Icons/ayurveda.png");
    uses = new LabelImage{"Uses", v2};
    uses->setMaximumSize(500, 220);
    herbInfo->addWidget(lff, 0, Qt::AlignCenter);
    herbInfo->addWidget(uses, 0, Qt::AlignCenter);

    rightSide->addWidget(image);
    rightSide->addLayout(herbInfo);
...
void HerbCard::paintEvent(QPaintEvent* e)
{
    QStyleOption option;
